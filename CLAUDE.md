...
<!-- wiki-chain
id: ai-cad-claude
status: Exporter pipeline stable (rotation-shatter fixed). Swap L/R relabels joints; "Mirror rig + mesh" button (mirror.js) physically mirrors geometry+skeleton across world X=0. Finger bones export explicit lmHand/lmPair. Guided setup wizard (CharacterWizard.jsx) walks bone review -> L/R check -> gated prep tools -> export. buildBoneRig() now validates every jointFrom/jointTo against a VALID_FK_JOINTS allowlist matching vtube's actual FKPositions runtime type (not worldFrame.ts's separate, richer CanonicalPose) before calling a bone driven — corrected a same-day mistake where Neck/Head were pointed at a "noseC" joint that doesn't exist in FKPositions, reintroducing the exact silently-broken-driven-bone bug being fixed; Hips/Head/ToeBase are genuinely locked, Spine chain hipMid->shMid, Neck shMid->headC. New detectFacingYawOffset() auto-corrects model facing (shoulder-line vs +Z, ±45° threshold) automatically on every model load, no manual click needed. Verified headlessly (tests/rig-integrity.test.mjs, two differently-named synthetic rigs) and live against real Soldier.glb export + userData.vtubeRig inspection. Companion runtime fallback + diagnostics severity changes landed in vtube's GlbBoneDriver.ts/rigDiagnostics.ts same day.
updated: 2026-07-05
