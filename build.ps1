...
.DESCRIPTION
  ...
  node/npm work on the local disk:
    ...
    3. `npm run build` there (vite build).
    4. Copy the resulting dist/ back to the project folder on Drive.

  Same pattern as the vtube project's build.ps1.
...
.PARAMETER Dev
  ...
  push new edits from Drive into it.

.PARAMETER Test
  Run the core engine tests (node --test) — these run fine from Drive too,
  the mirror is only needed for npm.
...
$SYNC_FILES = @("package.json", "package-lock.json", "vite.config.js", "eslint.config.js", "index.html")
$SYNC_DIRS  = @("src", "public", "tests")

Write-Host "Project : $ProjectRoot"
Write-Host "Mirror  : $MirrorRoot"

# ---- tests run fine directly on Drive (no npm involved) ---------------------
if ($Test) {
    node --test (Join-Path $ProjectRoot "tests/core.test.mjs") (Join-Path $ProjectRoot "tests/scenario.test.mjs")
...
Write-Host "Done. Fresh build is in $ProjectRoot\dist" -ForegroundColor Green
